...
Não é necessário clonar este repositório para operar o pipeline. Baixe ou importe os sete templates na interface do n8n e configure qualquer repositório GitHub como destino. A pasta `projects/` deste repositório é somente um conjunto de exemplos.
...
- credencial OpenAI;
- credencial Notion com leitura e credencial OpenAI para sumarização, caso as reuniões sejam usadas;
...
- [ ] Importar Core, Notion, IA, Publisher, Bootstrap, Formulário e Maintenance, nessa ordem, sem misturar variantes.
- [ ] Opcionalmente, colocar os sete no folder `githubDocs`.
- [ ] Configurar n8n API em `Listar workflows`.
- [ ] Configurar OpenAI em `Modelo OpenAI`.
- [ ] Configurar Notion API nos dois nodes HTTP e OpenAI API no modelo do agente resumidor, se a fonte for usada.
...
4. Abra o formulário autenticado, informe opcionalmente a página raiz do Notion e execute o Bootstrap.
